...
export class ManagePaymentsComponent implements OnInit {
  ...
  saveItem(index: number): void {
    ...
    if (item.paymentId === undefined) {
      /* Material snackbar instead of native alert for error feedback */
      this.showError('Cannot save: payment ID is undefined.');
      this.cancelEdit();
      return;
    }
  ...
  /* Delete a payment after confirmation — uses Material dialog instead of native confirm() */
  deleteItem(index: number): void {
    const item = this.dataSource.data[index];
    if (item.paymentId === undefined) {
      this.showError('Cannot delete: payment ID is undefined.');
      return;
    }
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Confirm Delete', message: `Delete payment #${item.paymentId}?` } as ConfirmDialogData,
    });
